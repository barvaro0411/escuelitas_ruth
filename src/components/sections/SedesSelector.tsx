    ...
    <section className="border-b border-border bg-surface-raised py-16 sm:py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto mb-10 max-w-3xl text-center">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-primary/10 px-4 py-2 text-xs font-extrabold uppercase tracking-wider text-primary">
            <MapPin className="h-4 w-4" aria-hidden="true" />
            Escuelas en Conchalí
          </div>
          <h2 className="mb-4 font-display text-3xl font-extrabold tracking-tight text-foreground sm:text-4xl">
            Elige la sede más cercana a tu hogar
          </h2>
          <p className="text-base font-semibold leading-relaxed text-muted sm:text-lg">
            Dos sedes, los mismos niveles educativos y orientación directa para tu familia.
          </p>
        </div>

        <div className="mx-auto grid max-w-5xl gap-5 md:grid-cols-2">
          {campuses.map((campus) => (
            <article key={campus.rbd} className="flex flex-col justify-between rounded-2xl border border-border bg-white p-6 shadow-sm transition-[border-color,box-shadow,transform] hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-md sm:p-8">
              <div>
                <div className="flex items-start justify-between gap-4">
                  <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-primary text-white">
                    <Building2 className="h-5 w-5" aria-hidden="true" />
                  </div>
                  <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-3 py-1 text-xs font-extrabold text-emerald-800">
              ...
              <div className="mt-8 flex flex-col gap-3 border-t border-border pt-6 sm:flex-row">
                <a
                  ...
                  className="inline-flex min-h-12 flex-1 items-center justify-center gap-2 rounded-xl bg-brand-yellow px-4 py-3 text-sm font-extrabold text-primary-dark transition-colors hover:bg-brand-yellow-light"
                >
        ...
        <div className="mx-auto mt-6 max-w-5xl rounded-2xl border border-primary/15 bg-white p-6 text-center sm:p-8">
          <p className="text-sm font-extrabold uppercase tracking-[0.14em] text-primary">Familias de comunas cercanas</p>
          <p className="mx-auto mt-2 max-w-3xl text-sm font-semibold leading-relaxed text-muted sm:text-base">
            También orientamos a familias de Huechuraba, Renca, Recoleta, Independencia, Quilicura y otros sectores del norte de Santiago.
          </p>
          <Link href="/matriculas-2027-santiago-norte" className="mt-4 inline-flex items-center gap-2 text-sm font-extrabold text-primary hover:text-primary-dark">
