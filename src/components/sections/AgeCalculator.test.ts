...
describe("calculateEligibility", () => {
  it.each([
    ...
    expect(result).toMatchObject({ status: "eligible", levelId, ageAtCutoff: { years } });
  });

  it("clasifica como menor a quien cumple tres años después del corte", () => {
    expect(calculateEligibility("2024-04-01")).toMatchObject({ status: "too_young" });
  });

  it("clasifica como mayor a quien ya cumplió seis años al corte", () => {
    expect(calculateEligibility("2021-03-31")).toMatchObject({ status: "too_old" });
  });

  it.each(["", "texto", "2024-02-30", "2028-01-01"])("rechaza la fecha inválida %s", (value) => {
