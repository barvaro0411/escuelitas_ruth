...
function estilo(tier: Tier, onDark: boolean) {
  if (tier === "matricular") {
    return `${BASE} min-h-14 rounded-xl bg-action px-7 py-4 text-base font-extrabold text-action-ink hover:bg-action-hover`;
  }
  if (tier === "evaluar") {
    return onDark
      ? `${BASE} min-h-14 rounded-xl border border-white/40 px-6 py-4 text-base text-white hover:bg-white/10`
      : `${BASE} min-h-14 rounded-xl border border-primary/25 px-6 py-4 text-base text-primary hover:border-primary hover:bg-surface-sunk`;
  }
  return onDark
    ? `${BASE} min-h-11 text-sm text-white/80 hover:text-white`
    : `${BASE} min-h-11 text-sm text-primary hover:text-primary-dark`;
}
...
export default function Cta({
  ...
  const content = (
    <>
      ...
      {children}
      {IconAfter && <IconAfter className="h-4 w-4 shrink-0" aria-hidden="true" />}
    </>
  );
